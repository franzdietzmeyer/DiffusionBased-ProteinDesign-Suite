#!/usr/bin/env python3
"""
Aggregate and analyze results from multiple runs WITH LIGAND
Prioritizes ipTM (protein-ligand interface quality)
...
def load_run_metrics(run_dir):
    ...
    for struct in data.get('structures', []):
        struct['run'] = run_name
        struct['ipTM'] = struct.get('ptm', 0)  # pTM from Chai is ipTM for ligand
        struct['pLDDT'] = struct.get('plddt', 0)
        struct['RMSD'] = struct.get('rmsd', 2.0) if 'rmsd' in struct else 2.0
        metrics.append(struct)

    return metrics

def calculate_aggregate_score_ligand(struct):
    """
    Chai's aggregate ranking for PROTEIN-LIGAND binding:
    Prioritizes interface quality (ipTM) over overall confidence
    ...
    score = 0.8 * ipTM + 0.2 * (pLDDT / 100.0)
...
def save_results_to_csv(all_metrics, top_100, output_dir, approach_name):
    ...
    all_csv = output_path / f'all_structures_{approach_name}_ligand.csv'
    with open(all_csv, 'w', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=['rank', 'run', 'protein', 'ipTM', 'pLDDT', 'RMSD', 'aggregate_score', 'filename'])
        writer.writeheader()
    ...
    # Save top 100 to CSV
    top100_csv = output_path / f'top100_structures_{approach_name}_ligand.csv'
...
def aggregate_runs(run_dirs, output_dir, approach_name):
    ...
    for struct in all_metrics:
        struct['aggregate_score'] = calculate_aggregate_score_ligand(struct)
    ...
    # Print summary
    print(f"\n{'='*80}")
    print(f"MULTI-RUN ANALYSIS WITH LIGAND: {approach_name}")
    print(f"{'='*80}")
    print(f"Total structures analyzed: {len(all_metrics)}")
    print(f"Runs included: {len(run_dirs)}")
    print(f"\nTOP 20 STRUCTURES:")
    print(f"{'Rank':<5} {'Run':<25} {'Protein':<35} {'ipTM':<8} {'pLDDT':<8} {'RMSD':<8} {'Score':<8}")
    print("-" * 100)

    for i, struct in enumerate(top_100[:20], 1):
        print(f"{i:<5} {struct['run']:<25} {struct['protein']:<35} "
              f"{struct['ipTM']:<8.4f} {struct['pLDDT']:<8.1f} {struct['RMSD']:<8.2f} {struct['aggregate_score']:<8.4f}")

    # Statistics
    print(f"\nOVERALL STATISTICS (all {len(all_metrics)} structures):")
    print(f"  Best ipTM: {max(s['ipTM'] for s in all_metrics):.4f}")
    print(f"  Mean ipTM: {np.mean([s['ipTM'] for s in all_metrics]):.4f}")
    ...
    for run_name in sorted(runs.keys()):
        ...
        mean_ipTM = np.mean([s['ipTM'] for s in run_metrics])
        mean_plddt = np.mean([s['pLDDT'] for s in run_metrics])

        print(f"  {run_name:30} Best Score: {best_score:.4f}  Mean: {mean_score:.4f}  ipTM: {mean_ipTM:.4f}  pLDDT: {mean_plddt:.1f}")
    ...
    top_100_data = {
        ...
        'ranking_formula': '0.8 × ipTM + 0.2 × (pLDDT/100) - RMSD_penalty',
        'top_100': [
            {
                'rank': i,
                'run': s['run'],
    ...
    json_file = output_path / f'top100_global_ranking_{approach_name}_ligand.json'
    ...
    # Create plots
    create_comparison_plots_ligand(all_metrics, top_100, output_path, approach_name)

    return all_metrics

def create_comparison_plots_ligand(all_metrics, top_100, output_dir, approach_name):
    """Create detailed comparison plots for ligand binding"""
    output_path = Path(output_dir)

    # Group by run
    runs = defaultdict(lambda: {'ipTM': [], 'pLDDT': [], 'RMSD': [], 'score': []})

    for struct in all_metrics:
        run = struct['run']
        runs[run]['ipTM'].append(struct['ipTM'])
    ...
    for (run_name, data), color in zip(sorted(runs.items()), colors):
        ...
        ax1.scatter(plddt_jitter, data['ipTM'],
                   label=run_name, s=80, alpha=0.5, color=color, edgecolors='black', linewidth=0.3)

    ax1.set_xlabel('pLDDT (Overall Confidence)', fontsize=13, fontweight='bold')
    ax1.set_ylabel('ipTM (Ligand Interface Quality)', fontsize=13, fontweight='bold')
    ...
    fig1.tight_layout()
    plt.savefig(output_path / f'all_structures_{approach_name}_ligand.png', dpi=300, bbox_inches='tight')
    print(f"✓ Saved all structures plot: {output_path / f'all_structures_{approach_name}_ligand.png'}")
    plt.close()

    # SEPARATE PLOT 2: TOP 100 ONLY (high resolution, colored by RMSD)
    fig2, ax2 = plt.subplots(figsize=(14, 10))
    top100_plddt = [s['pLDDT'] for s in top_100]
    top100_ipTM = [s['ipTM'] for s in top_100]
    top100_rmsd = [s['RMSD'] for s in top_100]

    scatter = ax2.scatter(top100_plddt, top100_ipTM, c=top100_rmsd, s=150, alpha=0.8,
    ...
    ax2.set_xlabel('pLDDT (Overall Confidence)', fontsize=13, fontweight='bold')
    ax2.set_ylabel('ipTM (Ligand Interface Quality)', fontsize=13, fontweight='bold')
    ax2.set_title(f'TOP 100 STRUCTURES: {approach_name}', fontsize=14, fontweight='bold')
    ax2.grid(True, alpha=0.3, linestyle='--')
    ax2.set_xlim([0, 105])
    ax2.set_ylim([-0.05, 1.05])
    fig2.tight_layout()
    plt.savefig(output_path / f'top100_structures_{approach_name}_ligand.png', dpi=300, bbox_inches='tight')
    print(f"✓ Saved top 100 structures plot: {output_path / f'top100_structures_{approach_name}_ligand.png'}")
    plt.close()
    ...
    # Plot 2: ipTM distribution per run (violin plot)
    ax4 = fig3.add_subplot(gs[0, 1])
    iptm_data = [runs[run]['ipTM'] for run in run_names]
    parts = ax4.violinplot(iptm_data, positions=range(len(run_names)), showmeans=True, showmedians=True)

    ax4.set_xticks(range(len(run_names)))
    ax4.set_xticklabels(run_names, rotation=45, ha='right')
    ax4.set_ylabel('ipTM (Ligand Interface Quality)', fontsize=11, fontweight='bold')
    ax4.set_title('ipTM Distribution per Run', fontsize=12, fontweight='bold')
    ...
    ax5.set_ylabel('pLDDT (Overall Confidence)', fontsize=11, fontweight='bold')
    ...
    plt.savefig(output_path / f'perrun_statistics_{approach_name}_ligand.png', dpi=300, bbox_inches='tight')
    print(f"✓ Saved per-run statistics plot: {output_path / f'perrun_statistics_{approach_name}_ligand.png'}")
    plt.close()

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Aggregate and analyze multiple runs WITH LIGAND')
    ...
    parser.add_argument('--name', default='ligand_design', help='Approach name for labeling')
